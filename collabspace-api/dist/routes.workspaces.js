...
export async function registerWorkspaceRoutes(app) {
    ...
    app.get("/workspaces/:workspaceId/members", { preHandler: [requireAuth] }, async (request, reply) => {
    ...
    app.post("/workspaces/:workspaceId/invite", { preHandler: [requireAuth] }, async (request, reply) => {
        const userId = request.user.sub;
        const { workspaceId } = request.params;
